...
void qrisc::simulate(const std::vector<Instr> &Instrs,
  ...
  for (auto Ins : Instrs) {
    ...
    if (Opcode == OPCODE_ADD) {
      ...
      Builder.CreateStore(V, getRegPtr(Ins.r1()));
    } else if (Opcode == OPCODE_UREM) {
      Value *V = Builder.CreateURem(getReg(Ins.r2()), getReg(Ins.r3()));
      Builder.CreateStore(V, getRegPtr(Ins.r1()));
    } else if (Opcode == OPCODE_ADDM) {
      /* addm, andm, etc may read from MMIO. Split them into __load +
       * calculation + store */
      Value *Addr =
          Builder.CreateAdd(getReg(Ins.r2()), Builder.getInt32(Ins.imm()));
      Value *Operand = Builder.CreateCall(LazyLoad, Addr);
      Value *V = Builder.CreateAdd(getReg(Ins.r1()), Operand);
      Builder.CreateStore(V, getRegPtr(Ins.r1()));
